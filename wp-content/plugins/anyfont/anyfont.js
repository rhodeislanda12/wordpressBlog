...
Control.ColorPicker = Class.create({
	initialize: function (element, options) {
		...
		this.swatch = new Element('div', {'style':'border:1px solid gray; position:absolute;right: 6px;top: 8px; width:14px; height: 14px; background-color:'+this.element.value});
...
// 				key == 'text-shadow' ? key = 'shadow' : 0;
	...
	insertCSSRule: function(){
		...
		$('css_custom_rule_block').insert({"bottom":new Element("input", {"type":"text", "name":'anyfont_new_'+this.rulenum+'[result]', "style":"width:217px;clear:both;margin-left:3px;"})});
...
var CheckboxStyle = Class.create({
	...
	initialize: function(parentEl){
		...
		this.parentEl.select("div.anyfont_checkbox").each(function(e){
			e.style.background = "transparent url("+af_set.url+"wp-content/plugins/anyfont/img/checkbox.gif) no-repeat scroll 200px 9px";
			if(e.hasClassName('anyfont_checkbox_on')){
				e.style.backgroundPosition = "200px -41px";
			} else {
				e.style.backgroundPosition = "200px 9px";
			}
			e.down('input').hide();
			e.observe("mousedown", function(event){
				...
				if(this.del.className == "anyfont_checkbox"){
					this.del.style.backgroundPosition = "200px -16px";
				} else {
					this.del.style.backgroundPosition = "200px -68px";
				}
			}.bind(this));
